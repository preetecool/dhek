...
import { getColumnName, getTargetColumnId } from "./project";
...
function applyGroupChange<T extends KanbanItemProps>(
  item: T,
  groupId: string
): T {
...
function getOverGroupId<
  T extends KanbanItemProps,
  C extends KanbanColumnDataProps,
>(overId: string, data: T[], columns: C[]): string | null {
...
function getCrossGroupInsertIndex<
  T extends KanbanItemProps,
  C extends KanbanColumnDataProps,
>(
  overId: string,
  ...
  if (isOverColumn) {
    const itemsInColumn = data.filter(
      (item) => getItemGroupId(item) === targetGroupId
    );
    if (itemsInColumn.length > 0) {
      const lastItemInColumn = itemsInColumn[itemsInColumn.length - 1];
      const lastItemIndex = data.findIndex(
        (item) => item.id === lastItemInColumn.id
      );
      ...
      if (enabled && over) {
        ...
        if (draggedItem) {
          ...
          if (overGroupId && draggedGroupId !== overGroupId) {
            ...
            const finalIndex = getCrossGroupInsertIndex(
              overId,
              data,
              columns,
              overGroupId,
            ...
            newData[activeIndex] = applyGroupChange(
              newData[activeIndex],
              overGroupId
            );
